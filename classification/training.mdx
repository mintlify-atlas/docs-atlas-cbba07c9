---
title: "Training the Crop-Weed Segmentation Model"
sidebarTitle: "Training"
description: "Train Bonnet or UNet on CWFID or BoniRob datasets using main.py. Configure dataset paths, batch size, epochs, and weighted categorical cross-entropy loss."
---
...
seg_model.compile(
    ...
    metrics=[
        'accuracy',
        iou_coef,
        keras_metrics.categorical_precision(label=0),   # Weed precision
        keras_metrics.categorical_recall(label=0),      # Weed recall
        keras_metrics.categorical_precision(label=1),   # Crop precision
        keras_metrics.categorical_recall(label=1),      # Crop recall
        keras_metrics.categorical_precision(label=2),   # Soil precision
        keras_metrics.categorical_recall(label=2),      # Soil recall
    ]
)
```
...
Mean IoU: 0.xxxx
Precision (label 0 - Weed): 0.xx   Recall (label 0 - Weed): 0.xx
Precision (label 1 - Crop): 0.xx   Recall (label 1 - Crop): 0.xx
Precision (label 2 - Soil): 0.xx   Recall (label 2 - Soil): 0.xx
```

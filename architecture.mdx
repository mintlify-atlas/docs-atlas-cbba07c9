---
title: "AGRIBOT System Architecture and Module Overview"
sidebarTitle: "Architecture"
description: "AGRIBOT is organized into four modules: robot modeling, autonomous navigation, crop-weed classification, and simulation — each implemented as ROS packages."
---
...
├── Crop_Weed_ Classification/   # CNN training & inference scripts
...
The navigation package, written in Python (`rospy`), contains five key scripts:
...
- **`data_manipulation.py`** — the `data_manipulation` node. Subscribes to `/magnetic` (`Vector3Stamped`) and applies a Moving Median filter followed by a single-dimension Kalman filter to produce a clean heading angle, published on `/angle`.

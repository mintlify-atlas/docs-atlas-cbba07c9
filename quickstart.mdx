...
<Steps>
  ...
  <Step title="Launch Gazebo with the farm world">
    ...
    `my_world.launch` loads `Farm.world`, starts `joint_state_publisher` and `robot_state_publisher`, and automatically includes `agribot_control.launch` so you do not need to start the controllers separately.
  </Step>

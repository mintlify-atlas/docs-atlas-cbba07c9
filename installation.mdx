...
```bash
pip3 install tensorflow==1.x keras numpy matplotlib opencv-python scikit-learn scikit-image Pillow imutils keras-metrics
```

<Note>
  The classification code in `Crop_Weed_Classification/model.py` was written for **TensorFlow 1.x and Keras 2.x**. Upgrading to TensorFlow 2.x will introduce breaking API changes — particularly around `keras.backend`, session handling, and model loading. Test compatibility thoroughly before upgrading your environment.
</Note>
